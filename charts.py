...
def make_insight(ticker: str,
    ...
    trend = "increased" if pct >= 0 else "declined"
    # flatten sentiment_list to scores
    import numpy as np
    scores = []
    if isinstance(sentiment_list, pd.Series):
        scores = sentiment_list.tolist()
    elif isinstance(sentiment_list, list):
        for v in sentiment_list:
            if isinstance(v, dict):
                # dict with "Score" or "score"
    ...
    s1 = f"{ticker} has {trend} {abs(pct)*100:.1f}% over the past 10 days."
