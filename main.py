...
from prediction import train_predict_stock, fetch_price_history
...
async def run_finbert_analysis(tickers: list, headlines_data: dict) -> dict:
    ...
    for ticker in tickers:
        try:
            ...
            # Fetch real price data for the ticker
            try:
                hist_df = fetch_price_history(ticker)
                if hist_df.empty or len(hist_df) < 10:
                    logger.warning(f"F04: Insufficient price data for {ticker}, using fallback")
                    price_data = pd.DataFrame({
                        'close': [100.0] * 30,
                        'date': pd.date_range(end=pd.Timestamp.now(), periods=30)
                    })
                else:
                    # Use actual close prices from history
                    price_data = pd.DataFrame({
                        'close': hist_df['Close'].values[-30:] if len(hist_df) >= 30 else hist_df['Close'].values,
                        'date': hist_df['Date'].values[-30:] if len(hist_df) >= 30 else hist_df['Date'].values
                    })
            except Exception as price_err:
                logger.warning(f"F04: Failed to fetch price data for {ticker}: {price_err}")
                price_data = pd.DataFrame({
                    'close': [100.0] * 30,
                    'date': pd.date_range(end=pd.Timestamp.now(), periods=30)
                })
            ...
            if len(closes) >= 14:
                ...
                # Simple MACD signal (EMA12 - EMA26)
                ema12 = pd.Series(closes).ewm(span=12).mean().iloc[-1]
                ema26 = pd.Series(closes).ewm(span=26).mean().iloc[-1] if len(closes) >= 26 else ema12
                macd_signal = (ema12 - ema26) / ema26 if ema26 != 0 else 0
                
                # Bollinger Band position (0=lower, 0.5=middle, 1=upper)
                sma20 = pd.Series(closes).rolling(20).mean().iloc[-1] if len(closes) >= 20 else closes[-1]
                std20 = pd.Series(closes).rolling(20).std().iloc[-1] if len(closes) >= 20 else 0.1
            ...
            technical_indicators = {
                ...
                'bb_position': float(max(0, min(1, bb_position))) if not pd.isna(bb_position) else 0.5
            }
...
def main():
    ...
    for t in all_tickers:
        try:
            ...
            # Create fallback empty prediction to prevent downstream KeyError
            preds[t] = {
                "predictions": [],
                "confidence": 0.0,
                "red_flag": False,
            ...
            forecast_data[t] = pd.DataFrame({"Date": [], "Forecast_Close": []})
            sentiment_map[t] = {}
