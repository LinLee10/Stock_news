...
def _backend_pool_diagnostics_html(report_input: DailyReportInput) -> str:
    ...
    return "\n".join(
        [
            ...
            f"packets: {int(diagnostics.get('llm_event_packets_after_filter', diagnostics.get('llm_briefing_event_packet_count', 0)))} "
            f"of {int(diagnostics.get('llm_event_packets_before_filter', 0))}; "
            f"low priority dropped: {int(diagnostics.get('llm_packets_dropped_low_priority', 0))}; "
            f"unclassified: {int(diagnostics.get('llm_unclassified_source_quality_count', 0))}; "
            f"multi-source: {int(diagnostics.get('llm_multisource_event_count', 0))}; "
            f"monitor candidates: {int(diagnostics.get('llm_top_monitor_candidate_count', 0))}; "
            f"score range: {escape(str(diagnostics.get('llm_attention_score_range') or {}))}; "
