    ...
    for tic in to_download:
            ...
            if os.path.exists(cache_path):
                try:
                    ...
                    if len(df) > 1:  # Has data beyond header
                        ...
                        df = df.sort_values("Date").reset_index(drop=True)
                        _bulk_price_cache[tic] = df
                        logger.info(f"Using old cached data for {tic} (daily limit reached)")
                        continue
                except Exception as e:
        ...
        try:
            ...
            if not raw_timeseries:
                ...
                if "rate limit" in info_msg.lower() or "rate limit" in note_msg.lower():
                    logger.warning(f"API rate limit reached. Using cached data for {tic} if available.")
...
def train_predict_stock(
    ...
    peer_df = fetch_price_history(ticker)       # using same ticker as peer (placeholder)
